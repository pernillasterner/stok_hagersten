// LOAD STUFF AFTER DOM IS LOADED
...
$(document).ready(function(){
		...
		if(getCookie('font-size-cookie')){
		    $fontsize = parseInt(getCookie('font-size-cookie'));
		    $('.col1').css('font-size',getCookie('font-size-cookie')+'px');
		}
		    ...
		    $('.plus').click(function(){
			    ...
			    setCookie('font-size-cookie',$fontsize);
	...
	$(".items2").cycle({
        ...
        // TRanslate Mailchimp label

        // Translate Mailchimp label{
        $('.mergeRow label:first-child').html('Vilket format föredrar du?');

	// TRIGGER THE HEIGHT OF THE COLORED BOXES ON THE START PAGE
	 setEqualHeight($(".first-page-puff-colored"));


	 /* CALENDAR DROPDOWN */
	 $('.calendar-select').click(function(){
	 	$('.calendar-drop-down').toggle();
    ...
    // ADD PIE CLASS TO ELEMNTS FOR IE CSS3 SUPPORT
    $('.main-menu').find('li').addClass('pie');
    $('.datepickerGoPrev').addClass('pie');
    $('.datepickerGoNext').addClass('pie');
    $('.datepickerMonth').addClass('pie');
    $('.switch-weeks').addClass('pie');


    // MAIN MENU SHOW SUBMENUES
    ...
    if ($(window).innerWidth() > 768) {
        ...
        $('.menu-item-has-children a').click(function(e) {
            var smenu = $(this).parent().find(".sub-menu").first();
            smenu.slideToggle();
            ...
            if (!$(e.currentTarget.parentElement.parentElement).hasClass('sub-menu')) {
                e.preventDefault();
            }
        });
    }
...
// LOAD STUGG AFTER EVERYTHING IS LOADED

$(window).load(function(){
	$('#browsable').show();
	$('#carousel').css('background','none');
	$('.carousel-mobile').css('background','none');
	$('.carousel-mobile').css('background','none');
...
// SET THE HEIGHT OF THE COLORED BOXES ON THE START PAGE
